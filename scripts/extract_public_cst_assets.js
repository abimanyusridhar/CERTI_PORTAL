...
const cssOutRel = path.join('public', 'assets', 'smg-public', 'cst', 'index.css');
const jsOutRel = path.join('public', 'assets', 'smg-public', 'cst', 'index.js');
...
if (!cssMatch) throw new Error('CSS <style> block not found in public/index.html');
...
html = html.replace(
  /<style>[\s\S]*?<\/style>/,
  '<link rel="stylesheet" href="/assets/smg-public/cst/index.css" />',
);

// 3) Extract main inline <script> immediately after the Cloudflare email-decode script.
// public/index.html contains: <script data-cfasync="false" ...email-decode...></script><script> ... </script>
const emailScriptMatch = html.match(
  /(<script[^>]*data-cfasync="false"[^>]*email-decode\.min\.js[^>]*><\/script>)(?:\s*|\n|\r)*<script>([\s\S]*?)<\/script>/,
);
if (!emailScriptMatch) {
  // Fallback: slightly broader match
  throw new Error('Email-decode script + following inline <script> not found in public/index.html');
}
...
html = html.replace(
  ...
  `${emailTag}<script src="/assets/smg-public/cst/index.js"></script>`,
);

fs.writeFileSync(htmlPath, html, 'utf8');
console.log('Extracted CST assets', { cssOutRel, jsOutRel });
