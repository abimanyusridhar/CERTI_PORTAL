 ...
 * Event bus + identity context for admin pages.
 * Loaded via dynamic import() by consumers (not a classic <script>, not
 * exposed on window) so a third-party script or XSS payload running in the
 * same page can't reach this module's state through a global reference —
 * only code that explicitly imports this exact module URL gets access.
 */
...
export function setPrincipal(identity) {
...
export function createPublisher(namespace) {
  return {
